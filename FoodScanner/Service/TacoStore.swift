...
final class TacoStore: ObservableObject {
    @Published var items: [TacoItem] = []
    
    init() {
        loadTacoCSV(named: "taco") // o arquivo taco.csv deve estar no bundle
    }
        ...
        do {
            ...
            for line in lines.dropFirst() { // pula o header
                ...
                if columns.count >= 5 {
                    let item = TacoItem(
                        nome: columns[0],
                        energiaKcal: Double(columns[1]) ?? 0,
                        proteina: Double(columns[2]) ?? 0,
                        lipideos: Double(columns[3]) ?? 0,
                        carboidratos: Double(columns[4]) ?? 0
                    )
                    items.append(item)
                }
            }
